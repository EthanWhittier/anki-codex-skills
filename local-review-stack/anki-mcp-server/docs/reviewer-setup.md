...
For the maintained local ticketed stack, call `get_next_due_card`, retain its machine-facing `reviewSessionId` and `reviewTicket`, then pass both to `rate_card_and_get_next` after an explicit rating. Never display either opaque value. Capability detection automatically retains the legacy path for stock AnkiConnect, but a maintained ticket-capable fork fails closed with `ANKI_VERSION_UNVERIFIED` until that exact Anki version passes the isolated FSRS compatibility gate.
