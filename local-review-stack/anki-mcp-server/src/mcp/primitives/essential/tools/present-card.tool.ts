...
export class PresentCardTool {
  ...
  @Tool({
    name: "present_card",
    description:
      "Retrieve a card's content for review. WORKFLOW: 1) Show question, 2) Wait for user answer, 3) Show answer with show_answer=true, 4) Evaluate and suggest rating (1-4), 5) When the user gives a rating, use rate_card.",
    parameters: z.object({
  ...
  async presentCard(
    ...
    try {
      ...
      const instruction = !showAnswer
        ...
        : "Answer revealed. Evaluate response and suggest a rating.";
