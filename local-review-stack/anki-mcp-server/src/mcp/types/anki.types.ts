...
export interface ChatReviewCapabilities {
  ...
  ankiVersion: string;
  ankiVersionVerified: boolean;
  testedAnkiVersions: string[];
  fsrs: {
    enabled: boolean;
    scheduler: "fsrs" | "sm2";
  };
...
export interface ChatReviewStatusResponse {
  active: boolean;
  generationHash: string;
  ownerHash?: string;
  cardId?: number;
...
export interface ChatReviewNextResponse {
  success: boolean;
  ticket: string | null;
...
export interface ChatReviewAnswerResponse {
  success: boolean;
  replayed: boolean;
  ratedCard: {
    cardId: number;
  ...
  next: ChatReviewNextResponse | null;
}
