...
1. **"Help me review my Spanish deck."** — The assistant syncs with AnkiWeb (`sync`), fetches one scheduler-selected card (`get_next_due_card`), keeps its opaque review ticket hidden, and consumes that ticket with `rate_card_and_get_next` only after your explicit rating.
...
- `get_next_due_card` / `rate_card_and_get_next` - Ticketed, scheduler-authoritative chat review when paired with the maintained local AnkiConnect fork; stock AnkiConnect uses the legacy compatibility path
